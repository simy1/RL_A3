...
def update_policy(states_list, actions_list, g_list, model, optimizer):
...
def runReinforceAlgo(env=None, model=None, optimizer=None, gamma=0.9, iterations=1000, print_details=False):
    iteration = 1
    ...
    while iteration <= iterations:
        ...
        # generate a trace
        states_list, actions_list, rewards_list, win_loss_ratio = generateTrace(env=env, model=model)
        if print_details:
        ...
        update_policy(states_list=states_list, actions_list=actions_list, g_list=g_list, model=model, optimizer=optimizer)
